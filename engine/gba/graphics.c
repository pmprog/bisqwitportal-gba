...
static u8 canvas_doublebuffer = 0;
...
void graphics_mode_bitmap(u8 DoubleBuffer)
{
    REG_DISPCNT = DCNT_MODE4 | DCNT_BG2 | DCNT_OBJ | DCNT_OBJ_1D;
    canvas_doublebuffer = DoubleBuffer;
...
void graphics_flip()
{
    ...
    if((REG_DISPCNT & DCNT_MODE4) == DCNT_MODE4 && canvas_doublebuffer != 0)
    {
        vid_page = (COLOR*)((u32)vid_page ^ VRAM_PAGE_SIZE);
        REG_DISPCNT ^= DCNT_PAGE;
    }
