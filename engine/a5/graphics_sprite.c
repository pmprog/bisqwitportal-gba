...
void graphics_sprites_configure(u8 Index, u16 Tile, ColourDepth BPP, u8 PaletteIndex, SpriteSize Size)
{
    ...
    switch(sprites[Index].graphic_size)
    {
        case SIZE_8x8:
            ...
            break;
        case SIZE_16x8:
            sprites[Index].cached_image = al_create_bitmap(16, 8);
            break;
        case SIZE_16x16:
            sprites[Index].cached_image = al_create_bitmap(16, 16);
            break;
        case SIZE_32x8:
            sprites[Index].cached_image = al_create_bitmap(32, 8);
            break;
        case SIZE_32x16:
            sprites[Index].cached_image = al_create_bitmap(32, 16);
            break;
        case SIZE_32x32:
        ...
        for(int tx = 0; tx < tilewidth; tx++)
        {
            for(int y = 0; y < 8; y++)
            {
                for(int x = 0; x < 8; x++)
                {
                    ...
                    p[b + 0] = (palidx == 0 ? 0 : canvas_palette[palidx].R);
                    p[b + 1] = (palidx == 0 ? 0 : canvas_palette[palidx].G);
                    p[b + 2] = (palidx == 0 ? 0 : canvas_palette[palidx].B);
